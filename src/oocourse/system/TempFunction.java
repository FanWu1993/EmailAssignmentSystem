...
public class TempFunction extends MyAbortableThread {
	
	final static boolean STUDENTIDIDENTIFICATIONONLY = false;

	HashMap<String, String> addressMap;
	
	public TempFunction(String mainPath, OOMailDelivery user,
			String homeworkIndex, TaskType taskType, boolean isAboutPrinciple, HashMap<String, String> addressMap) {
	...
	public void run() {
		...
		try {
			fileOutputStream = new FileOutputStream(new File(mainPath + "规格" + "in.txt"));
			user.addToProgressDisplay("列表名: " + "规格" + "in.txt");
		} catch (FileNotFoundException e1) {
			e1.printStackTrace();
			user.addToProgressDisplay("无法新建" + mainPath + "规格" + "in.txt");
			user.clearBusy();
			return;
		}
		...
		for(int i = 0; i < length; ++i) {
			...
			if(numberStartIndex >= 0 && isStudentID(fileNameList[i].substring(numberStartIndex)) &&
					(STUDENTIDIDENTIFICATIONONLY || fileNameList[i].contains("规格"))) {
